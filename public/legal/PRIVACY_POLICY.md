# Privacy Policy

**Last Updated:** April 9, 2026  
**Effective Date:** February 4, 2026

## 1. Introduction

Welcome to Aliseus ("we," "our," or "us"). We are committed to protecting your personal data and respecting your privacy rights. This Privacy Policy explains how we collect, use, store, and protect your information when you use our personal management platform.

Aliseus is a premium, AI-powered personal operating system that helps you manage your finances, life organization, and long-term goals. This policy complies with the General Data Protection Regulation (GDPR) and other applicable data protection laws.

## 2. Data Controller

**Aliseus**  
Email: privacy@aliseus.com  
Data Protection Officer: dpo@aliseus.com

For any questions about this Privacy Policy or our data practices, please contact our Data Protection Officer at the email address above.

## 3. Data We Collect

### 3.1 Account Information
- Email address
- Name
- User persona (Student, Freelancer, Professional, Family, Entrepreneur)
- Account credentials (encrypted)
- Profile preferences

### 3.2 Financial Data
- **Transactions:** Date, amount, category, subcategory, description, account information
- **Bank Accounts:** Account name, bank name, account type, balance, currency, credit limits
- **Budgets:** Budget categories, spending limits, budget periods
- **Debts:** Debt type, amounts, interest rates, payment schedules
- **Goals:** Financial goals, target amounts, deadlines
- **Assets:** Property information, cadastral references (if provided)
- **Investment Data:** Investment account balances and types

### 3.3 Life Management Data
- **Meal Planning:** Recipes, ingredients, dietary preferences, meal schedules
- **Pantry Management:** Inventory items, expiration dates
- **Travel Plans:** Trip details, itineraries, budgets
- **Shopping Lists:** Items, quantities, categories
- **Tasks and Notes:** Personal tasks and organizational notes

### 3.4 Family Information (Optional)
- Family member names
- Relationships
- Birth dates
- Contact information (email, phone)
- Growth records (for children)
- Weekly allowances

### 3.5 Usage and Analytics Data
- Device information (browser type, operating system)
- IP address
- Usage patterns and feature interactions
- Performance metrics (page load times, errors)
- Session duration and frequency
- Dashboard customization preferences

### 3.6 AI Interaction Data
- Prompts and queries submitted to AI features
- AI-generated suggestions and responses
- Feedback on AI recommendations

### 3.7 Technical Data
- Cookies and similar tracking technologies
- Log files
- Error reports and crash data
- Browser session information

## 4. Legal Basis for Processing

We process your personal data under the following legal bases:

### 4.1 Contractual Necessity
Processing necessary to provide our services, including:
- Account management
- Financial tracking and analysis
- AI-powered recommendations
- Data synchronization across devices

### 4.2 Legitimate Interests
- Improving our services and user experience
- Detecting and preventing fraud
- System security and integrity
- Analytics and performance optimization

### 4.3 Consent
- Marketing communications (where required)
- Optional features requiring additional data
- Cookies and tracking technologies (where required)

### 4.4 Legal Obligations
- Compliance with financial regulations
- Tax reporting requirements (where applicable)
- Response to legal requests

## 5. How We Use Your Data

We use your personal data for the following purposes:

### 5.1 Service Provision
- Provide core financial management features
- Generate AI-powered insights and recommendations
- Synchronize data across your devices
- Enable meal planning and life organization features
- Track budgets, debts, and financial goals

### 5.2 Service Improvement
- Analyze usage patterns to improve features
- Develop new functionality
- Optimize performance and user experience
- Conduct research and analytics

### 5.3 Communication
- Send service-related notifications
- Provide customer support
- Send updates about new features (with consent)
- Respond to your inquiries

### 5.4 Security and Compliance
- Detect and prevent fraud
- Ensure platform security
- Comply with legal obligations
- Enforce our Terms of Service

## 6. Data Sharing and Third Parties

We do not sell your personal data. We share data only in the following circumstances:

### 6.1 Service Providers
We work with trusted third-party service providers:

- **Supabase:** Backend infrastructure and authentication
- **Google Gemini Pro:** AI-powered features and recommendations
- **Vercel:** Hosting and deployment
- **Sentry:** Error tracking and monitoring
- **Vercel Analytics:** Usage analytics and performance monitoring
- **Plaid Technologies, Inc.:** Banking data connectivity. When you choose to link your bank accounts, we use Plaid to securely retrieve your account information, balances, and transaction history directly from your financial institution. Plaid acts as a data processor under our instruction. Your banking credentials are never transmitted to or stored by Aliseus — they are submitted directly to your bank through Plaid's encrypted infrastructure. Plaid's access to your data is governed by [Plaid's Privacy Policy](https://plaid.com/legal/) and Plaid's End User Privacy Policy. Bank connections made through Plaid comply with the Payment Services Directive 2 (PSD2) and expire after 180 days, at which point you will be prompted to re-authorise the connection.

All service providers are contractually obligated to protect your data and use it only for specified purposes.

### 6.2 Legal Requirements
We may disclose your data when required by law, including:
- Response to court orders or legal processes
- Protection of our rights and property
- Prevention of fraud or illegal activities
- Protection of user safety

### 6.3 Business Transfers
In the event of a merger, acquisition, or sale of assets, your data may be transferred to the acquiring entity, subject to the same privacy protections.

## 7. International Data Transfers

Your data may be processed in countries outside the European Economic Area (EEA). When we transfer data internationally, we ensure appropriate safeguards are in place:

- Standard Contractual Clauses (SCCs) approved by the European Commission
- Adequacy decisions by the European Commission
- Privacy Shield certification (where applicable)
- Other legally approved transfer mechanisms

## 8. Data Retention

We retain your personal data only as long as necessary:

### 8.1 Active Accounts
- Data is retained while your account is active
- Financial data: 7 years (for tax and legal compliance)
- Transaction history: Duration of account + 7 years
- Usage analytics: 2 years

### 8.2 Inactive Accounts
- Accounts inactive for 3 years may be archived
- You will receive notice before archival
- Archived data is retained for 1 additional year

### 8.3 Deleted Accounts
- Most data deleted within 30 days of account deletion
- Some data retained for legal compliance (up to 7 years)
- Anonymized analytics data may be retained indefinitely

## 9. Your Rights Under GDPR

You have the following rights regarding your personal data:

### 9.1 Right of Access
You can request a copy of all personal data we hold about you.

### 9.2 Right to Rectification
You can correct inaccurate or incomplete personal data.

### 9.3 Right to Erasure ("Right to be Forgotten")
You can request deletion of your personal data, subject to legal retention requirements.

### 9.4 Right to Restriction of Processing
You can request that we limit how we use your data in certain circumstances.

### 9.5 Right to Data Portability
You can receive your data in a structured, machine-readable format and transfer it to another service.

### 9.6 Right to Object
You can object to processing based on legitimate interests or for direct marketing purposes.

### 9.7 Right to Withdraw Consent
Where processing is based on consent, you can withdraw it at any time.

### 9.8 Right to Lodge a Complaint
You can file a complaint with your local data protection authority.

## 10. How to Exercise Your Rights

To exercise any of your rights, please contact us at:

**Email:** privacy@aliseus.com  
**Data Protection Officer:** dpo@aliseus.com

We will respond to your request within 30 days. You may be required to verify your identity before we process your request.

### Self-Service Options
Many rights can be exercised directly through your account settings:
- **Access:** Export your data from Settings > Privacy > Download My Data
- **Rectification:** Edit your information in Settings > Profile
- **Deletion:** Delete your account in Settings > Privacy > Delete Account
- **Data Portability:** Export data in JSON format from Settings > Privacy

## 11. Data Security

We implement industry-standard security measures to protect your data:

### 11.1 Technical Measures
- End-to-end encryption for sensitive data
- Encrypted data transmission (TLS/SSL)
- Encrypted data storage
- Regular security audits and penetration testing
- Multi-factor authentication support

### 11.2 Organizational Measures
- Access controls and role-based permissions
- Employee training on data protection
- Confidentiality agreements with staff and contractors
- Incident response procedures
- Regular backup and disaster recovery procedures

### 11.3 Your Responsibilities
- Keep your password secure and confidential
- Enable two-factor authentication
- Log out from shared devices
- Report suspicious activity immediately

## 12. Cookies and Tracking Technologies

We use cookies and similar technologies to enhance your experience:

### 12.1 Essential Cookies
Required for basic functionality:
- Authentication and session management
- Security features
- Load balancing

### 12.2 Analytics Cookies
Help us understand usage patterns:
- Vercel Analytics for performance monitoring
- Web Vitals for user experience metrics

### 12.3 Preference Cookies
Remember your settings:
- Dashboard layout preferences
- Theme and display settings
- Language preferences

### 12.4 Managing Cookies
You can control cookies through:
- Browser settings
- Our cookie consent banner
- Settings > Privacy > Cookie Preferences

Note: Disabling essential cookies may affect functionality.

## 13. Children's Privacy

Aliseus is not intended for children under 16 years of age. We do not knowingly collect personal data from children under 16.

The "Family Member" feature allows parents to track information about their children, but:
- Only parents/guardians can create and manage child profiles
- Children cannot create their own accounts
- Parents are responsible for the accuracy and appropriateness of data entered

If we become aware that we have collected data from a child under 16 without parental consent, we will delete it promptly.

## 14. AI and Automated Decision-Making

We use AI (Google Gemini Pro) to provide personalized recommendations:

### 14.1 AI Features
- Budget suggestions based on spending patterns
- Anomaly detection for unusual transactions
- Meal planning recommendations
- Financial forecasting and projections
- Savings opportunity identification

### 14.2 Your Rights
- You are not subject to solely automated decisions with legal or significant effects
- All AI recommendations are advisory only
- You maintain full control over all financial decisions
- You can opt out of AI features in Settings > AI Preferences

## 15. Changes to This Privacy Policy

We may update this Privacy Policy periodically to reflect:
- Changes in our practices
- Legal or regulatory requirements
- New features or services

### 15.1 Notification of Changes
- Material changes will be notified via email
- Notice will be posted in the application
- Continued use after changes constitutes acceptance
- You can review the "Last Updated" date at the top of this policy

### 15.2 Version History
Previous versions of this policy are available upon request.

## 16. Contact Information

For privacy-related questions or concerns:

**General Privacy Inquiries:**  
Email: privacy@aliseus.com

**Data Protection Officer:**  
Email: dpo@aliseus.com

**Support:**  
Email: support@aliseus.com

**Mailing Address:**  
Aliseus  
España

## 17. Supervisory Authority

If you are located in the EEA, you have the right to lodge a complaint with your local data protection authority. A list of EU data protection authorities can be found at: https://edpb.europa.eu/about-edpb/board/members_en

---

**Acknowledgment:** By using Aliseus, you acknowledge that you have read and understood this Privacy Policy and agree to the collection, use, and disclosure of your information as described herein.
