        ...
        <div className="bg-white dark:bg-onyx-900 p-6 rounded-[2rem] h-full flex flex-col border border-onyx-100 dark:border-onyx-800 shadow-sm hover:shadow-md transition-all">
            <div className="flex justify-between items-start mb-5">
                <div>
                    ...
                    <h3 className="text-lg font-black text-onyx-950 dark:text-white">Supermercados este Mes</h3>
                </div>
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
                <div className="text-center p-3 rounded-2xl bg-onyx-50 dark:bg-onyx-800/50">
                    <p className="text-[40px] font-black text-onyx-950 dark:text-white leading-none mb-1">{history.visits}</p>
                    <p className="text-[10px] font-black text-onyx-400 uppercase tracking-widest">Visitas</p>
                </div>
                <div className="text-center p-3 rounded-2xl bg-onyx-50 dark:bg-onyx-800/50">
                    <p className="text-[24px] font-black text-onyx-950 dark:text-white leading-none mt-2 pr-1">{history.totalSpent.toFixed(0)}€</p>
                    <p className="text-[10px] font-black text-onyx-400 uppercase tracking-widest mt-2">Total</p>
                </div>
            </div>

            <div className="space-y-2 flex-1">
                <p className="text-[10px] font-black text-onyx-400 uppercase tracking-widest mb-2">Últimas Compras</p>
                {history.recent.length === 0 ? (
                    <p className="text-xs font-bold text-onyx-300 italic text-center py-4">Sin registros este mes</p>
                ) : (
                    history.recent.map(tx => (
                        <div key={tx.id} className="flex justify-between items-center text-xs">
                            <span className="font-bold text-onyx-700 dark:text-onyx-300 truncate max-w-[120px]">
                                {tx.description || 'Supermercado'}
                            </span>
                            <span className="font-black text-onyx-900 dark:text-white">-{tx.amount.toFixed(2)}€</span>
                        </div>
                    ))
                )}
            </div>
