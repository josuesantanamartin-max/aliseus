        ...
        <div className="bg-white dark:bg-onyx-900 p-6 rounded-[2rem] h-full flex flex-col border border-onyx-100 dark:border-onyx-800 shadow-sm hover:shadow-md transition-all">
            <div className="flex justify-between items-start mb-5">
                <div>
                    ...
                    <h3 className="text-lg font-black text-onyx-950 dark:text-white">Caduca esta Semana</h3>
                </div>
            ...
            <div className="space-y-3 flex-1 overflow-y-auto">
                {expiringItems.length === 0 ? (
                    <div className="h-full flex flex-col items-center justify-center text-center py-4">
                        <AlertCircle className="w-8 h-8 text-onyx-200 dark:text-onyx-700 mb-2" />
                        <p className="text-xs font-bold text-onyx-400 dark:text-onyx-500">Todo fresco por ahora</p>
                    </div>
                ) : (
                    expiringItems.map(item => {
                        const days = getDaysRemaining(item.expiryDate!);
                        return (
                            <div key={item.id} className="group p-3 rounded-xl bg-onyx-50/50 dark:bg-onyx-800/50 border border-onyx-100 dark:border-onyx-700/50 hover:bg-white dark:hover:bg-onyx-800 hover:border-red-200 dark:hover:border-red-900/30 transition-all">
                                <div className="flex justify-between items-start mb-1">
                                    <h4 className="text-xs font-bold text-onyx-900 dark:text-white truncate pr-2">{item.name}</h4>
                                ...
                                <div className="flex items-center justify-between">
                                    <p className="text-[10px] text-onyx-500 dark:text-onyx-400">{item.quantity} {item.unit}</p>
                                    <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                        ...
                                        <button 
                                            ...
                                            className="p-1 px-2 bg-onyx-900 dark:bg-white text-white dark:text-onyx-900 rounded text-[8px] font-black flex items-center gap-1 transition-colors"
                                            title="Reponer"
                                        >
            ...
            <button 
                ...
                className="mt-4 w-full py-2.5 text-[10px] font-black uppercase tracking-widest text-onyx-500 dark:text-onyx-400 hover:text-red-600 dark:hover:text-red-400 border-t border-onyx-100 dark:border-onyx-800 transition-colors"
            >
