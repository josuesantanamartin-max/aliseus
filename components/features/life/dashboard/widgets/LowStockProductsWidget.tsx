        ...
        <div className="bg-white dark:bg-onyx-900 p-6 rounded-[2rem] h-full flex flex-col border border-onyx-100 dark:border-onyx-800 shadow-sm hover:shadow-md transition-all">
            <div className="flex justify-between items-start mb-5">
                <div>
                    ...
                    <h3 className="text-lg font-black text-onyx-950 dark:text-white">Bajo Mínimos</h3>
                </div>
            ...
            <div className="space-y-3 flex-1 overflow-y-auto">
                {lowStockItems.length === 0 ? (
                    <div className="h-full flex flex-col items-center justify-center text-center py-4">
                        <ShoppingCart className="w-8 h-8 text-onyx-200 dark:text-onyx-700 mb-2" />
                        <p className="text-xs font-bold text-onyx-400 dark:text-onyx-500">Despensa bien surtida</p>
                    </div>
                ) : (
                    lowStockItems.map(item => (
                        <div key={item.id} className="p-3 rounded-xl bg-onyx-50/50 dark:bg-onyx-800/50 border border-onyx-100 dark:border-onyx-700/50 flex items-center justify-between">
                            <div className="min-w-0 pr-2">
                                <h4 className="text-xs font-bold text-onyx-900 dark:text-white truncate">{item.name}</h4>
                                <div className="flex items-center gap-1.5 mt-0.5">
                                    <span className="text-[10px] font-black text-amber-600 dark:text-amber-400">
                                        {item.quantity} {item.unit}
                                    </span>
                                    <span className="text-[8px] font-bold text-onyx-400 dark:text-onyx-500 uppercase">
                            ...
                            <button 
                                ...
                                className="p-2 bg-onyx-900 dark:bg-white text-white dark:text-onyx-900 rounded-lg hover:scale-110 active:scale-95 transition-all shadow-sm"
            ...
            <button 
                ...
                className="mt-4 w-full py-2.5 text-[10px] font-black uppercase tracking-widest text-onyx-500 dark:text-onyx-400 hover:text-amber-600 dark:hover:text-amber-400 border-t border-onyx-100 dark:border-onyx-800 transition-colors"
            >
