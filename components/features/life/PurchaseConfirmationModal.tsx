...
export const PurchaseConfirmationModal: React.FC<PurchaseConfirmationModalProps> = ({
    ...
    return (
        ...
        >
            <div
                className="bg-white dark:bg-aliseus-900 rounded-[2.5rem] shadow-2xl w-full max-w-lg overflow-hidden"
                onClick={(e) => e.stopPropagation()}
            >
                {/* Header */}
                <div className="px-8 py-6 border-b border-gray-100 dark:border-aliseus-800 flex justify-between items-center bg-gradient-to-r from-emerald-50 to-green-50 dark:from-emerald-900/20 dark:to-green-900/20">
                ...
                <div className="p-8 space-y-6 max-h-[60vh] overflow-y-auto custom-scrollbar">
                    {/* Items Summary */}
                    <div className="bg-gray-50 dark:bg-aliseus-800/50 rounded-2xl p-5 border border-gray-100 dark:border-aliseus-700">
                    ...
                    <div>
                        ...
                        <div className="relative">
                            <input
                                ...
                                disabled={isProcessing}
                                className="w-full p-5 pr-12 bg-white dark:bg-aliseus-800 border-2 border-gray-200 dark:border-aliseus-700 rounded-2xl font-bold text-2xl text-gray-900 dark:text-white focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 dark:focus:ring-emerald-900/50 transition-all disabled:opacity-50"
                                placeholder="0.00"
                                autoFocus
                            />
                            ...
                            {accounts.map((account) => (
                                <button
                                    ...
                                    className={`w-full p-4 rounded-2xl border-2 transition-all text-left flex items-center justify-between disabled:opacity-50 ${selectedAccountId === account.id
                                        ...
                                        : 'border-gray-200 dark:border-aliseus-700 bg-white dark:bg-aliseus-800 hover:border-emerald-300'
                                        }`}
                                >
                                    <div className="flex items-center gap-3">
                                        <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${selectedAccountId === account.id
                                            ? 'bg-emerald-500'
                                            : 'bg-gray-200 dark:bg-aliseus-700'
                                            }`}>
                ...
                {/* Footer */}
                <div className="p-6 bg-gray-50 dark:bg-aliseus-800 border-t border-gray-100 dark:border-aliseus-700 flex gap-3">
                    <button
                        onClick={handleClose}
                        disabled={isProcessing}
                        className="flex-1 px-6 py-4 bg-white dark:bg-aliseus-700 text-gray-700 dark:text-gray-300 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-gray-100 dark:hover:bg-aliseus-600 transition-all disabled:opacity-50 border border-gray-200 dark:border-aliseus-600"
                    >
                        Cancelar
                    </button>
                    <button
