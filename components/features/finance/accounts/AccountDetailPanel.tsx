...
export const AccountDetailPanel: React.FC<AccountDetailPanelProps> = ({
    ...
    return (
        <div className="space-y-8">
            <div className="bg-white p-8 lg:p-10 rounded-3xl shadow-sm border border-aliseus-100 relative overflow-hidden group hover:shadow-lg transition-all duration-500">
                <div className="flex justify-between items-start mb-8 relative z-10">
                    <div>
                        <p className="text-xs font-bold text-aliseus-400 uppercase tracking-[0.2em] mb-1">{account.bankName || getTypeLabel(account.type)}</p>
                        ...
                        {(account as any).iban && <p className="text-xs font-mono text-aliseus-300 mt-2">{(account as any).iban}</p>}
                    </div>
                    <div className="flex gap-3">
                        <button onClick={() => onEdit(account)} className="p-3 bg-aliseus-50 hover:bg-aliseus-100 rounded-xl text-aliseus-500 hover:text-cyan-900 transition-colors"><Pencil className="w-5 h-5" /></button>
                ...
                {stats && (
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6 relative z-10">
                        {[
                            ...
                            <div key={s.label} className="bg-aliseus-50/50 p-4 md:p-6 rounded-2xl border border-aliseus-100/50">
                                <p className="text-[10px] font-bold text-aliseus-400 uppercase tracking-widest mb-3">{s.label}</p>
                                <div className="flex items-end justify-between gap-2">
                                    <div>
                                        <p className="text-xs font-semibold text-aliseus-400 mb-1">{s.prevLabel}</p>
                                        <p className="text-base md:text-lg font-bold text-aliseus-600">{formatEUR(s.prev)}</p>
                                    </div>
                ...
                <div className="flex justify-between items-end mt-8 pt-8 border-t border-aliseus-100 relative z-10">
                    <div>
                        <p className="text-[10px] font-bold text-aliseus-400 uppercase tracking-widest mb-2">Saldo Actual</p>
            ...
            {linkedCards.length > 0 && (
                <div className="space-y-3">
                    <h4 className="text-sm font-bold text-aliseus-400 uppercase tracking-widest">Tarjetas vinculadas</h4>
            ...
            {linkedCards.length === 0 && account.type !== 'CREDIT' && account.type !== 'DEBIT' && (
                <div className="bg-aliseus-50/30 p-8 rounded-3xl border border-dashed border-aliseus-100 flex flex-col items-center justify-center text-center">
                    <CreditCard className="w-10 h-10 text-aliseus-200 mb-3" />
                    <p className="text-xs font-bold text-aliseus-400 uppercase tracking-widest">No hay tarjetas asociadas</p>
