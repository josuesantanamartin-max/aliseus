    ...
    return (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-cyan-950/40 backdrop-blur-md p-4 animate-fade-in">
            <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden max-h-[90vh] flex flex-col">
                <div className="px-8 py-5 flex justify-between items-center border-b border-aliseus-50 sticky top-0 bg-white z-10">
                    ...
                    <button onClick={onClose} className="text-aliseus-400 hover:text-cyan-900 p-2 hover:bg-aliseus-50 rounded-xl"><X className="w-5 h-5" /></button>
                </div>
                <form onSubmit={handleSubmit} className="p-8 space-y-4 overflow-y-auto">
                    <div>
                        <label className="block text-[10px] font-bold text-aliseus-400 uppercase tracking-widest mb-2">Nombre</label>
                    ...
                    <div>
                        <label className="block text-[10px] font-bold text-aliseus-400 uppercase tracking-widest mb-2">Descripción</label>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="block text-[10px] font-bold text-aliseus-400 uppercase tracking-widest mb-2">Fecha Inicio</label>
                            <input type="date" required value={pStart} onChange={e => setPStart(e.target.value)}
                                className="w-full p-4 bg-aliseus-50 border border-aliseus-100 rounded-2xl font-bold text-sm outline-none focus:border-cyan-300 focus:bg-white transition-colors" />
                        </div>
                        <div>
                            <label className="block text-[10px] font-bold text-aliseus-400 uppercase tracking-widest mb-2">Fecha Fin</label>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="block text-[10px] font-bold text-aliseus-400 uppercase tracking-widest mb-2">Tipo de Proyecto</label>
                        ...
                        <div>
                            <label className="block text-[10px] font-bold text-aliseus-400 uppercase tracking-widest mb-2">Fecha Clave (Objetivo)</label>
                    ...
                    <div>
                        <label className="block text-[10px] font-bold text-aliseus-400 uppercase tracking-widest mb-2">Responsable Principal</label>
                    ...
                    {pType === 'EVENTO_FAMILIAR' && (
                        <div className="p-5 bg-gradient-to-br from-cyan-50 to-white border border-cyan-100 rounded-3xl space-y-4 animate-in fade-in slide-in-from-top-2 duration-300">
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <div>
                                    <label className="block text-[9px] font-bold text-aliseus-400 uppercase tracking-widest mb-1.5 ml-1">Lugar</label>
                                    <div className="relative">
                                        <MapPin className="absolute left-3.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-aliseus-300" />
                                        <input type="text" value={pLocation} onChange={e => setPLocation(e.target.value)} placeholder="Ej. Restaurante..."
                                            className="w-full pl-9 pr-4 py-3 bg-white border border-aliseus-100 rounded-xl text-xs font-bold outline-none focus:border-cyan-300 transition-colors" />
                                    </div>
                                </div>
                                <div>
                                    <label className="block text-[9px] font-bold text-aliseus-400 uppercase tracking-widest mb-1.5 ml-1">Invitados</label>
                                    <div className="relative">
                                        <Users className="absolute left-3.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-aliseus-300" />
                            ...
                            <div>
                                <label className="block text-[9px] font-bold text-aliseus-400 uppercase tracking-widest mb-1.5 ml-1">Importancia / Urgencia</label>
                                <div className="flex gap-2">
                                    {(['BAJA', 'MEDIA', 'ALTA'] as const).map(imp => (
                                        <button key={imp} type="button" onClick={() => setPImportance(imp)}
                                            className={`flex-1 py-2.5 rounded-xl text-[9px] font-black uppercase tracking-widest border transition-all ${pImportance === imp ? 'bg-cyan-600 text-white border-cyan-600 shadow-sm' : 'bg-white border-aliseus-100 text-aliseus-400 hover:border-cyan-200'}`}>
                                            {imp}
                                        </button>
                                    ))}
                                </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            <label className="block text-[10px] font-bold text-aliseus-400 uppercase tracking-widest mb-3 text-right">Acento Visual</label>
                            <div className="flex gap-1.5 flex-wrap justify-end">
                                {PRESET_COLORS.map(c => (
                                    <button key={c} type="button" onClick={() => setPColor(c)}
                                        className={`w-6 h-6 rounded-full transition-all ${pColor === c ? 'scale-110 ring-2 ring-offset-2 ring-aliseus-800 shadow-md' : 'hover:scale-105 opacity-80 hover:opacity-100'}`}
