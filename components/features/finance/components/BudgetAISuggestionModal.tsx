...
export const BudgetAISuggestionModal: React.FC<BudgetAISuggestionModalProps> = ({
    ...
    return (
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-cyan-900/60 backdrop-blur-md p-4 animate-fade-in">
            <div className="bg-white rounded-Aliseus shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh]">
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar p-8 bg-aliseus-50/50">
                    {smartSuggestions.length > 0 ? (
                        <div className="space-y-4">
                            {smartSuggestions.map((suggestion, idx) => {
                                const period = (suggestion as any).period || 'MONTHLY';
                                return (
                                    <div key={idx} className={`p-4 rounded-xl border transition-all duration-300 cursor-pointer flex items-center justify-between group ${suggestion.isSelected ? 'bg-white border-cyan-500 shadow-md ring-1 ring-cyan-500' : 'bg-white border-aliseus-100 hover:border-cyan-200'}`} onClick={() => {
                                        const newSugg = [...smartSuggestions];
                                        newSugg[idx].isSelected = !newSugg[idx].isSelected;
                                        setSmartSuggestions(newSugg);
                                    }}>
                                        <div className="flex items-center gap-4">
                                            <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center transition-colors shrink-0 ${suggestion.isSelected ? 'bg-cyan-500 border-cyan-500' : 'border-aliseus-200 bg-white'}`}>
                                            ...
                                            <div>
                                                ...
                                                <div className="flex items-center gap-2 mt-1">
                                                    <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded uppercase tracking-wider ${period === 'MONTHLY' ? 'bg-aliseus-100 text-aliseus-600' : 'bg-aliseus-900 text-white'}`}>
                                                        {period === 'MONTHLY' ? 'Mensual' : 'Anual'}
                                                    </span>
                                                    <p className="text-xs text-aliseus-500">
                                        ...
                                        <div className="text-right">
                                            <div className="text-xs font-bold text-aliseus-400 uppercase tracking-widest mb-1">Sugerido</div>
                        ...
                        <div className="text-center py-10">
                            <p className="text-aliseus-400 font-bold">No tengo suficientes datos históricos para generar sugerencias.</p>
                        </div>
                    )}
                </div>

                <div className="p-6 bg-white border-t border-aliseus-100 flex justify-end gap-4 z-20 shrink-0">
                    <button onClick={() => setIsAutoModalOpen(false)} className="px-6 py-3 text-aliseus-400 font-bold text-xs uppercase tracking-widest hover:text-aliseus-800 transition-colors">Cancelar</button>
