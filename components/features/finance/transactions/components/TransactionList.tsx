                ...
                <AnimatedListItem key={date}>
                    <div className="animate-fade-in group/day">
                        <div className="flex items-center gap-6 mb-8 px-2">
                            <div className="h-[1px] flex-1 bg-aliseus-100/50 dark:bg-white/10"></div>
                            <h4 className="text-[11px] font-bold text-aliseus-400 dark:text-aliseus-500 uppercase tracking-[0.3em] whitespace-nowrap bg-white dark:bg-aliseus-900 px-6 py-2 rounded-full border border-aliseus-100 dark:border-white/10 shadow-sm transition-all group-hover/day:border-cyan-100 group-hover/day:text-cyan-600 group-hover/day:scale-105">
                                {new Date(date).toLocaleDateString('es-ES', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })}
                            </h4>
                            <div className="h-[1px] flex-1 bg-aliseus-100/50 dark:bg-white/10"></div>
                        </div>

                        <div className="bg-white dark:bg-aliseus-900 rounded-Aliseus shadow-sm border border-aliseus-100 dark:border-white/5 overflow-hidden divide-y divide-aliseus-50 dark:divide-white/5 relative">
                            {groupedTransactions[date].map((t: Transaction) => (
                                <div key={t.id} onClick={() => onEdit(t)} className="p-8 hover:bg-aliseus-50/50 dark:hover:bg-white/5 transition-all duration-300 group/item flex flex-col sm:flex-row items-center gap-8 cursor-pointer border-l-4 border-l-transparent hover:border-l-cyan-600">
                                    {/* Icon */}
                                    ...
                                    <div className="flex-1 text-center sm:text-left">
                                        ...
                                        <div className="flex flex-wrap items-center justify-center sm:justify-start gap-4">
                                            <div className="flex items-center gap-2 px-3 py-1.5 bg-aliseus-50 dark:bg-white/5 border border-aliseus-100 dark:border-white/10 rounded-xl font-bold text-[10px] text-aliseus-500 dark:text-aliseus-400 uppercase tracking-widest group-hover/item:bg-white dark:group-hover/item:bg-aliseus-800 transition-colors">
                                                {t.category}
                                                {t.subCategory && <span className="opacity-40">/</span>}
                                                {t.subCategory && <span>{t.subCategory}</span>}
                                            </div>
                                            <div className="text-aliseus-400 dark:text-aliseus-500 font-bold text-[11px] flex items-center gap-3 bg-white dark:bg-aliseus-800 px-3 py-1.5 rounded-xl border border-aliseus-100/50 dark:border-white/10 group-hover/item:border-cyan-100 dark:group-hover/item:border-cyan-500/50 transition-colors">
                                    ...
                                    <div className="flex items-center gap-10 pr-2">
                                        <div className="text-right">
                                            ...
                                            <p className="text-[10px] font-bold text-aliseus-400 dark:text-aliseus-500 uppercase tracking-widest mt-1 opacity-0 group-hover/item:opacity-100 transition-opacity">Ver Detalle</p>
                                        </div>
                                        <div className="flex gap-3 opacity-0 group-hover/item:opacity-100 translate-x-4 group-hover/item:translate-x-0 transition-all duration-500">
                                            <button onClick={(e) => { e.stopPropagation(); onDelete(t.id); }} className="p-3.5 bg-white dark:bg-aliseus-800 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-xl text-aliseus-400 dark:text-aliseus-500 hover:text-red-500 dark:hover:text-red-400 border border-aliseus-100 dark:border-white/10 hover:border-red-100 dark:hover:border-red-500/30 transition-all shadow-sm active:scale-95"><Trash2 className="w-5 h-5" /></button>
                                        </div>
                                    </div>
                                </div>
                            ))}
