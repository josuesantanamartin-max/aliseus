...
const NotificationItem: React.FC<{
    ...
    return (
        <motion.div
            ...
            className={`relative flex flex-col gap-2 p-3 rounded-xl border-l-4 ${styles.border} ${styles.bg} ${!n.read ? 'ring-1 ring-inset ring-aliseus-100 dark:ring-aliseus-800' : 'opacity-70'}`}
        >
            {/* Header */}
            <div className="flex items-start justify-between gap-2">
                <div className="flex items-center gap-2 flex-1 min-w-0">
                    <CategoryIcon category={n.category} className={`w-3.5 h-3.5 flex-shrink-0 ${styles.icon}`} />
                    <p className="text-[12px] font-bold text-aliseus-900 dark:text-white truncate">{n.title}</p>
                    {!n.read && <span className="w-1.5 h-1.5 rounded-full bg-indigo-500 flex-shrink-0" />}
                </div>
                <div className="flex items-center gap-1 flex-shrink-0">
                    <span className="text-[10px] text-aliseus-400">{timeAgo(n.createdAt)}</span>
                    <button
                        onClick={() => onDismiss(n.id)}
                        className="w-5 h-5 flex items-center justify-center rounded-full hover:bg-aliseus-200 dark:hover:bg-aliseus-700 transition-colors text-aliseus-400 hover:text-aliseus-700 dark:hover:text-white"
            ...
            {/* Body */}
            <p className="text-[11px] text-aliseus-500 dark:text-aliseus-400 leading-relaxed">{n.message}</p>
    ...
    const renderGroup = (label: string, items: AliseusNotification[]) => {
        ...
        return (
            <div className="space-y-2">
                <p className="text-[9px] font-black uppercase tracking-[0.2em] text-aliseus-400 dark:text-aliseus-600 px-1">{label}</p>
                {items.map((n) => (
                    <NotificationItem
                        key={n.id}
                        n={n}
            ...
            {isOpen && (
                <>
                    ...
                    <motion.div
                        ...
                        className="fixed top-0 left-72 h-full w-80 bg-white dark:bg-aliseus-950 border-r border-aliseus-100 dark:border-aliseus-800 z-40 flex flex-col shadow-2xl"
                    >
                        {/* Header */}
                        <div className="flex items-center justify-between px-5 py-5 border-b border-aliseus-100 dark:border-aliseus-800 flex-shrink-0">
                            <div className="flex items-center gap-2">
                                <span className="text-sm font-bold text-aliseus-900 dark:text-white">{tt(language, 'title')}</span>
                                {unreadCount() > 0 && (
                            ...
                            <div className="flex items-center gap-1">
                                {unreadCount() > 0 && (
                                    <button
                                        ...
                                        className="p-1.5 rounded-lg text-aliseus-400 hover:text-indigo-600 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 transition-colors"
                                    >
                                ...
                                <button
                                    onClick={onClose}
                                    className="p-1.5 rounded-lg text-aliseus-400 hover:text-aliseus-700 hover:bg-aliseus-100 dark:hover:bg-aliseus-800 transition-colors"
                                >
                                    <X className="w-4 h-4" />
                                </button>
                            </div>
                        ...
                        <div className="flex-1 overflow-y-auto custom-scrollbar p-4 space-y-4">
                            {notifications.length === 0 ? (
                                ...
                                >
                                    ...
                                    <p className="text-sm font-bold text-aliseus-800 dark:text-aliseus-200">{tt(language, 'empty')}</p>
                                    <p className="text-[11px] text-aliseus-400 max-w-[180px]">{tt(language, 'emptyMsg')}</p>
                                </motion.div>
                            ) : (
                        ...
                        <div className="h-6 bg-gradient-to-t from-white dark:from-aliseus-950 to-transparent flex-shrink-0 pointer-events-none -mt-6" />
                    </motion.div>
                </>
            )}
        </AnimatePresence>
